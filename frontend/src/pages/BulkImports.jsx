...
export default function BulkImports() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setImports(data); setVendors(vends);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setToast({ message: 'Import created', type: 'success' });
  ...
  const getStatusColor = (s) => s === 'completed' ? 'success' : s === 'processing' ? 'info' : s === 'failed' ? 'danger' : 'warning';

  return (
    <div>
      <div className="page-header">
        <div><h2>Bulk Imports</h2><p>{imports.length} import records</p></div>
      ...
      <div className="dashboard-grid">
        ...
        <div className="stat-card">
          ...
          <h4>Processing</h4>
          <div className="stat-value">{imports.filter(i => i.status === 'processing').length}</div>
        </div>
        <div className="stat-card">
            ...
            <tbody>
              ...
              imports.map(i => (
                <tr key={i.id} onClick={() => setSelected(i)}>
                  ...
                  <td><span className={i.duplicates_found > 0 ? 'badge badge-warning' : ''}>{i.duplicates_found}</span></td>
                  <td><span className={i.errors > 0 ? 'badge badge-danger' : ''}>{i.errors}</span></td>
                  <td><span className={`badge badge-${getStatusColor(i.status)}`}>{i.status}</span></td>
      ...
      {selected && (
        <div className="detail-panel">
          ...
          <div className="detail-body">
            ...
            <div className="detail-field"><label>Status</label><span className={`badge badge-${getStatusColor(selected.status)}`}>{selected.status}</span></div>
            <div className="detail-field"><label>Total Rows</label><span>{selected.total_rows}</span></div>
            <div className="detail-field"><label>Processed</label><span>{selected.processed_rows}</span></div>
            <div className="detail-field">
              <label>Progress</label>
              <div className="ai-score-bar">
                <div className="ai-score-fill" style={{ width: `${selected.total_rows > 0 ? (selected.processed_rows / selected.total_rows * 100) : 0}%`, background: '#10b981' }} />
              </div>
              <span style={{ fontSize: 12, color: '#64748b' }}>{selected.total_rows > 0 ? (selected.processed_rows / selected.total_rows * 100).toFixed(0) : 0}%</span>
            </div>
            <div className="detail-field"><label>Duplicates Found</label><span style={{ fontSize: 20, fontWeight: 700, color: '#f59e0b' }}>{selected.duplicates_found}</span></div>
            <div className="detail-field"><label>Errors</label><span style={{ fontSize: 20, fontWeight: 700, color: '#ef4444' }}>{selected.errors}</span></div>
