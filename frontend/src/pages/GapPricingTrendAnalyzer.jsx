...
export default function GapPricingTrendAnalyzer() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-pricing-trend-analyzer', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, marginBottom: 6 }}>Pricing Trend Analyzer</h1>
      <p style={{ color: '#555', marginBottom: 20 }}>(anomaly detection on prices)</p>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          ...
          >
            {loading ? 'Running...' : 'Run Pricing Trend Analyzer'}
          </button>
          <button
