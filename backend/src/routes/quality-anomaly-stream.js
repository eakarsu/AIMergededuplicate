// Auto-generated feature route: quality-anomaly-stream
// Domain: data quality streaming scorer — Capability: streaming data-quality scoring and anomaly flagging
// Mounted under: /api/quality-anomaly-stream
...
// POST /api/quality-anomaly-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a data quality streaming scorer. Your task is streaming data-quality scoring and anomaly flagging. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/quality-anomaly-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/quality-anomaly-stream/:id — fetch one
...
// DELETE /api/quality-anomaly-stream/:id — discard a run
