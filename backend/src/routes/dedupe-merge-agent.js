// Auto-generated feature route: dedupe-merge-agent
// Domain: agentic dedupe + merge resolver — Capability: resolving conflicts using name + image + SKU signals
// Mounted under: /api/dedupe-merge-agent
...
// POST /api/dedupe-merge-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic dedupe + merge resolver. Your task is resolving conflicts using name + image + SKU signals. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/dedupe-merge-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/dedupe-merge-agent/:id — fetch one
...
// DELETE /api/dedupe-merge-agent/:id — discard a run
