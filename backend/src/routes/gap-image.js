...
// Route: /api/gap-image
// Feature: Image
// Description: Image deduplication pipeline
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Image. Image deduplication pipeline' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'image', title: 'Image', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'image', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['image']);
      return res.json({ slug: 'image', items: r.rows || [] });
    }
    return res.json({ slug: 'image', items: [] });
  } catch (e) {
    return res.json({ slug: 'image', items: [], error: (e && e.message) || 'Server error' });
  }
});
