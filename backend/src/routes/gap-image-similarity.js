...
// Route: /api/gap-image-similarity
// Feature: Image Similarity
// Description: (vision-based dupe matching)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Image Similarity. (vision-based dupe matching)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Image Similarity\nDescription: (vision-based dupe matching)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['image-similarity', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'image-similarity', title: 'Image Similarity', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'image-similarity', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['image-similarity']);
      return res.json({ slug: 'image-similarity', items: r.rows || [] });
    }
    return res.json({ slug: 'image-similarity', items: [] });
  } catch (e) {
    return res.json({ slug: 'image-similarity', items: [], error: (e && e.message) || 'Server error' });
  }
});
