// Auto-generated feature route: vision-product-enrich
// Domain: product vision enricher — Capability: enriching product data from photos
// Mounted under: /api/vision-product-enrich
...
// POST /api/vision-product-enrich/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a product vision enricher. Your task is enriching product data from photos. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-product-enrich/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-product-enrich/:id — fetch one
...
// DELETE /api/vision-product-enrich/:id — discard a run
