...
// Feature: Inventory
// Description: Inventory sync (real-time stock)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Inventory. Inventory sync (real-time stock)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'inventory', title: 'Inventory', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'inventory', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['inventory']);
      return res.json({ slug: 'inventory', items: r.rows || [] });
    }
    return res.json({ slug: 'inventory', items: [] });
  } catch (e) {
    return res.json({ slug: 'inventory', items: [], error: (e && e.message) || 'Server error' });
  }
});
