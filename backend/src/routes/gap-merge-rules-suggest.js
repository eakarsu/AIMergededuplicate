...
// Route: /api/gap-merge-rules-suggest
// Feature: Merge Rules Suggest
// Description: (suggest merge criteria from corpus)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Merge Rules Suggest. (suggest merge criteria from corpus)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Merge Rules Suggest\nDescription: (suggest merge criteria from corpus)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['merge-rules-suggest', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'merge-rules-suggest', title: 'Merge Rules Suggest', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'merge-rules-suggest', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['merge-rules-suggest']);
      return res.json({ slug: 'merge-rules-suggest', items: r.rows || [] });
    }
    return res.json({ slug: 'merge-rules-suggest', items: [] });
  } catch (e) {
    return res.json({ slug: 'merge-rules-suggest', items: [], error: (e && e.message) || 'Server error' });
  }
});
