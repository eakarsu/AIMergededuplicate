...
// Route: /api/gap-supplier
// Feature: Supplier
// Description: Supplier data validation rules engine
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Supplier. Supplier data validation rules engine' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'supplier', title: 'Supplier', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'supplier', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['supplier']);
      return res.json({ slug: 'supplier', items: r.rows || [] });
    }
    return res.json({ slug: 'supplier', items: [] });
  } catch (e) {
    return res.json({ slug: 'supplier', items: [], error: (e && e.message) || 'Server error' });
  }
});
