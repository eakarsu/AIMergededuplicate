// Auto-generated feature route: vertical-templates
// Domain: vertical data template librarian — Capability: producing category-aware validation rules
// Mounted under: /api/vertical-templates
...
// POST /api/vertical-templates/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a vertical data template librarian. Your task is producing category-aware validation rules. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vertical-templates/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vertical-templates/:id — fetch one
...
// DELETE /api/vertical-templates/:id — discard a run
