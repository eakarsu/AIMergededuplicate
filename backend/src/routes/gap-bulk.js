...
// Route: /api/gap-bulk
// Feature: Bulk
// Description: Bulk edit workflow UI
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Bulk. Bulk edit workflow UI' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'bulk', title: 'Bulk', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'bulk', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['bulk']);
      return res.json({ slug: 'bulk', items: r.rows || [] });
    }
    return res.json({ slug: 'bulk', items: [] });
  } catch (e) {
    return res.json({ slug: 'bulk', items: [], error: (e && e.message) || 'Server error' });
  }
});
