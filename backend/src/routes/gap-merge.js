...
// Route: /api/gap-merge
// Feature: Merge
// Description: Merge preview / approval workflow
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Merge. Merge preview / approval workflow' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'merge', title: 'Merge', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'merge', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['merge']);
      return res.json({ slug: 'merge', items: r.rows || [] });
    }
    return res.json({ slug: 'merge', items: [] });
  } catch (e) {
    return res.json({ slug: 'merge', items: [], error: (e && e.message) || 'Server error' });
  }
});
