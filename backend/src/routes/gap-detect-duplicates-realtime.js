...
// Route: /api/gap-detect-duplicates-realtime
// Feature: Detect Duplicates Realtime
// Description: (live duplicate flag on insert)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Detect Duplicates Realtime. (live duplicate flag on insert)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Detect Duplicates Realtime\nDescription: (live duplicate flag on insert)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['detect-duplicates-realtime', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'detect-duplicates-realtime', title: 'Detect Duplicates Realtime', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'detect-duplicates-realtime', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['detect-duplicates-realtime']);
      return res.json({ slug: 'detect-duplicates-realtime', items: r.rows || [] });
    }
    return res.json({ slug: 'detect-duplicates-realtime', items: [] });
  } catch (e) {
    return res.json({ slug: 'detect-duplicates-realtime', items: [], error: (e && e.message) || 'Server error' });
  }
});
