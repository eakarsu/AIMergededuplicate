...
// Route: /api/gap-pricing-trend-analyzer
// Feature: Pricing Trend Analyzer
// Description: (anomaly detection on prices)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Pricing Trend Analyzer. (anomaly detection on prices)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Pricing Trend Analyzer\nDescription: (anomaly detection on prices)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['pricing-trend-analyzer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'pricing-trend-analyzer', title: 'Pricing Trend Analyzer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'pricing-trend-analyzer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['pricing-trend-analyzer']);
      return res.json({ slug: 'pricing-trend-analyzer', items: r.rows || [] });
    }
    return res.json({ slug: 'pricing-trend-analyzer', items: [] });
  } catch (e) {
    return res.json({ slug: 'pricing-trend-analyzer', items: [], error: (e && e.message) || 'Server error' });
  }
});
